...
:build_minified_version
    ...
    echo   Creating build
    echo.
    java -jar "%YUI_FOLDER%\yuicompressor-%YUI_COMPRESSOR_VERSION%.jar" --charset=%YUI_CHARSET% --type=%YUI_TYPE% %YUI_NOMUNGE% "%YUI_FOLDER%\full.%YUI_TYPE%" -o "%YUI_FOLDER%\min.%YUI_TYPE%"
...
:copy_and_remove
    ...
    echo.
    rem ;; uncomment to add built datetime: echo./** %YUI_BUILD_NAME% - %DATE% %TIME% */>"%YUI_FOLDER%\%YUI_BUILD_NAME%.min.%YUI_TYPE%"
...
:show_file_list_and_append_content
    ...
    for /f "delims=" %%G in (%YUI_FILE_LIST%) do (
        ...
        if "%YUI_CHECK_UTF8_BOM%"=="1" call:check_utf8_bom %bom%
        if "%YUI_DEBUG%"=="1" (
            java -jar "%YUI_FOLDER%\yuicompressor-%YUI_COMPRESSOR_VERSION%.jar" --charset=%YUI_CHARSET% --type=%YUI_TYPE% %YUI_NOMUNGE% "%YUI_FOLDER%\tmp" -o "%YUI_FOLDER%\tmp.min"
